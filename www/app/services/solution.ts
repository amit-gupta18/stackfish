...
import { readProblemAssets } from './problemData';
...
export async function calculateFullSolution(problem: string, solution: string): Promise<string> {
  try {
    const { fullInputPath } = readProblemAssets(problem);
    if (!fullInputPath) {
      throw new Error(`Problem ${problem} does not have a full input file`);
    }
...
export function hasFullInput(problem: string): boolean {
  return readProblemAssets(problem).hasFullInput;
}

...
export class ProblemService {
  ...
  private static async getAttackVector(problemName: string, model: Model, parentLog: Log | null, onLogUpdate: (log: Log) => void, tests: SyntheticTest[] = []): Promise<void> {
    ...
    for (let i = 0; i < postAttackVectorSolutionCallsPerModel[model]; i++) {
      this.getSolution(problemName, model, log, onLogUpdate, result.attack_vector, result.tags, tests)
    }
    log.status = 'done';
    onLogUpdate(log);
  }
  ...
  private static async getSyntheticTests(problemName: string, model: Model, onLogUpdate: (log: Log) => void): Promise<void> {
    ...
    for (let i = 0; i < postSyntheticTestCallsPerModel[model]; i++) {
      this.getAttackVector(problemName, model, log, onLogUpdate, result.tests);
      this.getSolution(problemName, model, log, onLogUpdate, undefined, undefined, result.tests)
    }
    onLogUpdate(log);
  }
  ...
  private static async runAllTests(
    ...
    if (result?.skipped && result.reason === 'missing_full_input') {
      log.sub_tasks = [
        ...(log.sub_tasks || []),
        {
          id: generateId(),
          depth: log.depth + 1,
          message: '📦 No full input available, finishing after QA validation',
          status: 'done'
        }
      ];
      log.status = 'done';
  ...
  static async startProcess(problemName: string, model: Model, onLogUpdate: (log: Log) => void) {
    this.checkIfOnlyOneOutputValid(problemName, onLogUpdate)

    for (let i = 0; i < directSolutionCallsPerModel[model]; i++) {
      this.getSolution(problemName, model, null, onLogUpdate)
    }

    for (let i = 0; i < attackVectorCallsPerModel[model]; i++) {
      this.getAttackVector(problemName, model, null, onLogUpdate)
    }

    for (let i = 0; i < syntheticTestCallsPerModel[model]; i++) {
      this.getSyntheticTests(problemName, model, onLogUpdate)
    }

...
const bucketName = process.env.WORKER_GCS_BUCKET || "hacker-bucket";
const useGcs = process.env.WORKER_USE_GCS !== "false";
...
async function downloadFromGCS(objectUrl, destPath) {
  if (!useGcs) {
    throw new Error("Received inputUrl but WORKER_USE_GCS is disabled");
  }
